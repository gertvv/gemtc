...
test_that("TSD2 example 1 (dichotomous data, binomial/logit)", {
  ...
  data <- mtc.data.studyrow(read.table('studyrow/tsd2-1.data.txt', header=TRUE), treatmentNames=treatments)
  expect_that(data, equals(dget('studyrow/tsd2-1.out.txt')))
})
...
test_that("TSD2 example 3 (count data, binomial/cloglog)", {
  ...
  data <- mtc.data.studyrow(
    read.table('studyrow/tsd2-3.data.txt', header=TRUE),
    studyVars=c('time'='time'),
    studyNames=studies, treatmentNames=treatments)
  expect_that(data, equals(dget('studyrow/tsd2-3.out.txt')))
})
...
test_that("TSD2 example 5 (continuous data, normal/identity)", {
  data <- mtc.data.studyrow(
    read.table('studyrow/tsd2-5.data.txt', header=TRUE),
    armVars=c('treatment'='t', 'mean'='y', 'std.err'='se'))
  expect_that(data, equals(dget('studyrow/tsd2-5.out.txt')))
})
...
test_that("TSD2 example 7 (relative effect data)", {
  data <- mtc.data.studyrow(
    read.table('studyrow/tsd2-7.data.txt', header=TRUE),
    armVars=c('treatment'='t', 'mean'='y', 'std.err'='se'),
    studyVars=c('var'='V'))
  expect_that(data, equals(dget('studyrow/tsd2-7.out.txt')))
})

test_that("TSD2 example 8 (mixed data)", {
  data1 <- mtc.data.studyrow(
    read.table('studyrow/tsd2-8.data1.txt', header=TRUE),
    armVars=c('treatment'='t.a', mean='y.a', 'std.err'='se.a'),
    nArmsVar='na.a')
  expect_that(data1, equals(dget('studyrow/tsd2-8.out1.txt')))

  data2 <- mtc.data.studyrow(
    read.table('studyrow/tsd2-8.data2.txt', header=TRUE),
    armVars=c('treatment'='t', mean='y', 'std.err'='se'),
    nArmsVar='na',
    studyNames=4:7)
  expect_that(data2, equals(dget('studyrow/tsd2-8.out2.txt')))
})
